...
function BookingCard({ booking, therapistIndex, bookingIndexInTherapist, onBookingClick }) {
  ...
  const handleDragStart = useCallback((e) => {
    ...
    // Position ghost image so cursor is at the top of the card.
    // This way, dropClientY on drop = where the card top should go.
    e.dataTransfer.setDragImage(e.currentTarget, e.currentTarget.offsetWidth / 2, 0);
  }, [booking.id]);
  ...
  return (
    <div
      ...
      className={`absolute left-0 right-0 mx-1 rounded-md cursor-move overflow-hidden ${
        isSelected
    ...
    >
      ...
      */}
      {/* Move grip icon — top-right corner */}
      <div className="absolute top-0.5 right-0.5 opacity-40 hover:opacity-80 text-[8px] leading-none cursor-grab" title="Drag to move">
        ⋮⋮
      </div>
